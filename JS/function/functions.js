...
export function keystone_point_calculator(level, best, time) {
  ...
  if (time != 0) {
    ...
    if (time >= 0 && time <= 39) {
      keystone_point = keystone_point + (bonus_timer_point / bonus_max_percent) * time;
    } else if (time <= 0 && time >= -40) {
      keystone_point = keystone_point - (loose_timer_point / loose_max_percent) * Math.abs(time);
    } else if (time <= -40) {
      return 0;
    }
  }
  ...
  const diff_input = document.getElementById("difference");
  const html_diff = document.getElementById("difference_result");
  const html_new_score = document.getElementById("new_score");
  const rio_score = document.getElementById("rio_score").textContent;
